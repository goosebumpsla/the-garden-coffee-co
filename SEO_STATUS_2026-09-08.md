...
- Production still resolves through Netlify. The live exact-source audit fails because Netlify is serving the older pinned `ceb77a8` deployment instead of current GitHub `main`. Auto publishing remains on, but Netlify skips every new production deployment with “account credit usage exceeded” because the team exhausted its current billing-cycle credits. No paid upgrade was made.
